...
export function isApplicable(
  ...
  if (
    transaction.endingMonthId !== undefined &&
    new Date(transaction.endingMonthId) < monthDate
  ) {
    return false
  }
...
export function findRecurringTransactionsForMonth<
  T extends RecurringTransaction
>(monthId: string, transaction: T[]): T[] {
  return transaction.filter((transaction) => isApplicable(monthId, transaction))
}

export function mapExpensesToCategories<T extends Expense>(
  expenses: T[],
  categories: Category[]
): CategoryMapping<T>[] {
...
export function sumExpenses<T extends Expense>(
  categoryMappings: CategoryMapping<T>[]
): (CategoryMapping<T> & HasValue)[] {
